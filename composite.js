...
export function composite(concepts) {
  ...
  function merge(comp, incr, source) {
    function mergeprops(into, from) {
      const keys = Object.keys(into).concat(Object.keys(from)).filter(uniq);
    ...
    incr.nodes.forEach((node, id) => {
      const match = comp.nodes.find(
        (each) => each.type == node.type && each.props.name == node.props.name
      );
      if (match) {
