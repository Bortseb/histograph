...
//from https://github.com/WardCunningham/graph/blob/main/docs/collaborator/dotify.js
export function dotify(complex) {
  ...
  const nodes = graph.nodes.map((node, id) => {
    ...
    return `${id} [label="${label}" ${color} ${
      node.props.url || node.props.tick
        ? `URL="${node.props.url || "#"}" target="_blank"`
        : ""
    } tooltip="${tip(node.props)}"]`;
  });
  const edges = graph.rels.map((rel) => {
    const color = emphasis[rel.type] || "";
    return `${rel.from}->${rel.to} [label="${
      rel.type
    }" ${color} labeltooltip="${tip(rel.props)}"]`;
  });
  return [
    "digraph {",
